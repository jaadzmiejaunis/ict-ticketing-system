    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            <div class="p-6 border-b border-gray-800">
                                <div class="flex justify-between items-center mb-4">
                                    ...
                                    <form action="{{ route('notifications.readAll') }}" method="POST">
                                        @csrf
                                        <button type="submit" class="text-[10px] font-black text-indigo-400 hover:text-indigo-300 uppercase tracking-widest">Mark All Read</button>
                                    </form>
                                </div>
                                <div class="flex p-1 bg-gray-900/50 rounded-xl gap-1">
                                    ...
                                    <button @click="tab = 'tickets'" :class="tab === 'tickets' ? 'bg-white text-gray-900' : 'text-gray-400 hover:text-gray-200'" class="flex-1 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all">Tickets</button>
                                </div>
                            </div>
                            ...
                            <div class="max-h-96 overflow-y-auto custom-scrollbar bg-[#111827]">
                                @php
                                    ...
                                    $ticketAlerts = $allNotifications->filter(fn($n) => !in_array($n->data['type'] ?? '', ['mention', 'reply']));
                                @endphp
                                ...
                                <template x-if="tab === 'all' || tab === 'tickets'">
                                    <div>
                                        @foreach($ticketAlerts as $notification)
                                            @php $isUnread = $notification->unread(); @endphp
                                            <a href="{{ route('tickets.show', $notification->data['ticket_id'] ?? $notification->data['id'] ?? '#') }}"
                                               class="flex items-center gap-4 p-5 hover:bg-gray-800/50 transition border-b border-gray-800/50 group {{ $isUnread ? '' : 'opacity-60' }}">
                                                <div class="w-10 h-10 rounded-xl bg-gray-800 flex items-center justify-center text-[10px] font-black text-gray-500 group-hover:text-indigo-400 border border-transparent transition uppercase">TI</div>
                                                <div class="flex-1">
                                                    <div class="flex justify-between items-center">
                                                        <span class="text-xs font-black {{ $isUnread ? 'text-gray-200' : 'text-gray-500' }} group-hover:text-white uppercase tracking-tight">{{ $notification->data['title'] ?? 'Ticket Updated' }}</span>
                                                        <span class="text-[9px] text-gray-600 font-bold">{{ $notification->created_at->diffForHumans(null, true) }}</span>
                                                    </div>
                                                    <p class="text-[10px] text-gray-500 font-bold truncate mt-0.5">#{{ $notification->data['ticket_id'] ?? '' }}: {{ $notification->data['ticket_title'] ?? '' }}</p>
                                                </div>
                                            </a>
                                        @endforeach
                                    </div>
                            ...
                            <a href="{{ route('notifications.index') }}" class="block py-4 text-center text-[10px] font-black text-white bg-gray-800 hover:bg-gray-700 transition uppercase tracking-[0.2em]">
                                View all history
                            </a>
                        </div>
                    </div>
                ...
                @else
                    <a href="{{ route('login') }}" class="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2 rounded-md font-bold text-sm uppercase transition shadow-md">
                        {{ __('Log In') }}
                    </a>
                @endauth
            </div>
